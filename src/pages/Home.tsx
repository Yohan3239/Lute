...
export default function Home() {
  ...
  const now = Date.now();
  // ---- Today Stats ----
  const dueToday = cards.filter((c) => c.nextReview <= now && c.status !== "new").length;
  ...
  const deckStats = decks.map((deck) => {
    ...
    return {
      ...
      due: deckCards.filter((c) => c.nextReview <= now && c.status !== "new").length,
    ...
    <div className="text-gray-100 p-6 space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="p-4 bg-[#111113] border border-white/5 rounded-lg shadow-[0_0_30px_-10px_rgba(129,140,248,0.2)]">
          <div className="text-4xl font-bold hover:scale-105">{dueToday}</div>
          <div className="opacity-70 text-gray-400 text-lg">Due Now</div>
          <div className="text-sm tracking-wide text-gray-400/70">All decks</div>
        </div>
            ...
            >
              <div className="flex flex-row justify-between">
                ...
                <div className="opacity-70 mt-2 text-sm text-gray-400">
                  {deck.due} due  {deck.new > 0 ? `| +${deck.new} new` : ""} 
                </div>
              </div>
              {defaultMode === "none" ? (
