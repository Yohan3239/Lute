...
export async function decrementCoins(amount: number = 1): Promise<void> {
  const { error } = await supabase.rpc('decrement_coins', {amount });
  if (error) {
...
export async function TopupCoins() {
  const { error } = await supabase.rpc('claim_monthly_coins');
  if (error) {
    console.error("Error topping up coins:", error);
  } else {
    console.log((`Topped up coins for user`));
    window.dispatchEvent(new CustomEvent('coinsDecremented'));
  }
}

export async function getIsProUser(): Promise<boolean> {
  const { data, error } = await supabase.rpc('get_is_pro');
  if (error) {
